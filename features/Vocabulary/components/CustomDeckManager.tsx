...
const buildQueryVariants = (query: string): QueryVariants => ({
  raw: normalize(query),
  kana: normalize(toKana(query)),
  romaji: normalize(toRomaji(query)),
});

const buildSearchTerms = (vocabObj: IVocabObj): string[] => [
...
const matchesQuery = (
  ...
  if (!queryVariants.raw) return true;

  return searchTerms.some(
    term =>
      term.includes(queryVariants.raw) ||
      term.includes(queryVariants.kana) ||
      term.includes(queryVariants.romaji),
  );
};
...
const CustomDeckManager = () => {
  ...
  const [indexedVocabObjs, setIndexedVocabObjs] = useState<IndexedVocabObj[]>(
    [],
  );
  ...
  useEffect(() => {
    const loadAllVocab = async () => {
      ...
      const dedupedWords = dedupeWords(allLevels.flat());
      setIndexedVocabObjs(
        dedupedWords.map(vocabObj => ({
          vocabObj,
          searchTerms: buildSearchTerms(vocabObj),
        })),
      );
    };
  ...
  const queryVariants = useMemo(() => buildQueryVariants(deckSearch), [deckSearch]);

  const filteredResults = useMemo(
    () =>
      indexedVocabObjs
        .filter(indexedVocabObj =>
          matchesQuery(indexedVocabObj.searchTerms, queryVariants),
        )
        .map(indexedVocabObj => indexedVocabObj.vocabObj)
        .slice(0, SEARCH_RESULTS_LIMIT),
    [indexedVocabObjs, queryVariants],
  );
  ...
  const toggleDraftVocab = (vocabObj: IVocabObj) => {
    setDraftSelection(prev => {
      ...
      if (isCurrentlySelected) {
        return prev.filter(currentVocab => currentVocab.word !== vocabObj.word);
      }

      return dedupeWords([...prev, vocabObj]);
    });
  };
  ...
  const saveDeck = () => {
    ...
    playClick();
    if (!editingDeckId) {
      createCustomDeck(trimmedName.slice(0, DECK_NAME_CHAR_LIMIT), draftSelection);
    } else {
      updateCustomDeckName(editingDeckId, trimmedName.slice(0, DECK_NAME_CHAR_LIMIT));
